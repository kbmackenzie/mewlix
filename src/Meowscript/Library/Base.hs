...
module Meowscript.Library.Base
( baseLibrary
) where
...
import Meowscript.Library.Utils
-- IO:
import Meowscript.IO.Print (printText, printTextLn, printError)
import qualified Data.Text.IO as TextIO

{- Base -}
baseLibrary :: (MonadIO m) => m (Environment MeowAtom)
...
snoop = asks (MeowList . map MeowString . meowArgs . meowState)

search :: Evaluator Prim
search = lookUp "x" >>= \case
    (MeowString x) -> let hasDef def = Map.member def . meowDefines . meowState
